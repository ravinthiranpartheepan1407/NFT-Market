import React from "react";

function Arkhammtimeline(){
  return(
    <div>
    <h1 className="xl:text-5xl md:text-4xl text-2xl font-semibold leading-tight text-center text-white sm:mb-0 mb-12"> Our Launch Timeline </h1>
    <ol className="bg-background items-center p-28 sm:flex">
      <li className="relative mb-6 sm:mb-0">
      <div className="flex items-center">
          <div className="flex z-10 justify-center items-center w-6 h-6 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 rounded-full ring-0 ring-white dark:bg-blue-900 sm:ring-8 dark:ring-gray-900 shrink-0">
          </div>
          <div className="hidden sm:flex w-full bg-gray-200 h-0.5 dark:bg-gray-700"></div>
      </div>
      <div className="mt-3 sm:pr-8">
          <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">NFT Minting</h3>
          <br />
          <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Launched on June 01</time>
          <p className="text-base font-normal text-gray-500 dark:text-gray-400">Arkhamm NFT Minting Engine.</p>
      </div>
      </li>
      <li className="relative mb-6 sm:mb-0">
      <div className="flex items-center">
          <div className="flex z-10 justify-center items-center w-6 h-6 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 rounded-full ring-0 ring-white dark:bg-blue-900 sm:ring-8 dark:ring-gray-900 shrink-0">
          </div>
          <div className="hidden sm:flex w-full bg-gray-200 h-0.5 dark:bg-gray-700"></div>
      </div>
      <div className="mt-3 sm:pr-8">
          <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">NFT Market</h3>
            <br />
          <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Launched on June 01</time>
          <p className="text-base font-normal text-gray-500 dark:text-gray-400">Arkhamm NFT Marketplace.</p>
      </div>
      </li>
      <li className="relative mb-6 sm:mb-0">
      <div className="flex items-center">
          <div className="flex z-10 justify-center items-center w-6 h-6 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 rounded-full ring-0 ring-white dark:bg-blue-900 sm:ring-8 dark:ring-gray-900 shrink-0">
          </div>
          <div className="hidden sm:flex w-full bg-gray-200 h-0.5 dark:bg-gray-700"></div>
      </div>
      <div className="mt-3 sm:pr-8">
          <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">AKM Token</h3>
            <br />
          <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Launched on June 01</time>
          <p className="text-base font-normal text-gray-500 dark:text-gray-400">Arkhamm Initial Coin Offerings.</p>
      </div>
      </li>
      <li className="relative mb-6 sm:mb-0">
      <div className="flex items-center">
          <div className="flex z-10 justify-center items-center w-6 h-6 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 rounded-full ring-0 ring-white dark:bg-blue-900 sm:ring-8 dark:ring-gray-900 shrink-0">
          </div>
          <div className="hidden sm:flex w-full bg-gray-200 h-0.5 dark:bg-gray-700"></div>
      </div>
      <div className="mt-3 sm:pr-8">
          <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">De-Cloud</h3>
            <br />
          <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Launching on June 31</time>
          <p className="text-base font-normal text-gray-500 dark:text-gray-400">Arkhamm Cloud Service Launch.</p>
      </div>
      </li>
      <li className="relative mb-6 sm:mb-0">
      <div className="flex items-center">
          <div className="flex z-10 justify-center items-center w-6 h-6 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 rounded-full ring-0 ring-white dark:bg-blue-900 sm:ring-8 dark:ring-gray-900 shrink-0">
          </div>
          <div className="hidden sm:flex w-full bg-gray-200 h-0.5 dark:bg-gray-700"></div>
      </div>
      <div className="mt-3 sm:pr-8">
          <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">DEX Launch</h3>
            <br />
          <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Launched on June 31</time>
          <p className="text-base font-normal text-gray-500 dark:text-gray-400">Arkhamm De-Centralized Finance.</p>
      </div>
      </li>
      <li className="relative mb-6 sm:mb-0">
      <div className="flex items-center">
          <div className="flex z-10 justify-center items-center w-6 h-6 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 rounded-full ring-0 ring-white dark:bg-blue-900 sm:ring-8 dark:ring-gray-900 shrink-0">
          </div>
          <div className="hidden sm:flex w-full bg-gray-200 h-0.5 dark:bg-gray-700"></div>
      </div>
      <div className="mt-3 sm:pr-8">
          <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">DAO Launch</h3>
            <br />
          <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Launched on June 31</time>
          <p className="text-base font-normal text-gray-500 dark:text-gray-400">Arkhamm De-Centralized Autonomous Org.</p>
      </div>
      </li>
      <li className="relative mb-6 sm:mb-0">
      <div className="flex items-center">
          <div className="flex z-10 justify-center items-center w-6 h-6 bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 rounded-full ring-0 ring-white dark:bg-blue-900 sm:ring-8 dark:ring-gray-900 shrink-0">
          </div>
          <div className="hidden sm:flex w-full bg-gray-200 h-0.5 dark:bg-gray-700"></div>
      </div>
      <div className="mt-3 sm:pr-8">
          <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">Game Launch</h3>
            <br />
          <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">Launching on December 01</time>
          <p className="text-base font-normal text-gray-500 dark:text-gray-400">Arkhamm De-Centralized Exchange.</p>
      </div>
      </li>
    </ol>

    </div>

  )
}

export default Arkhammtimeline;
